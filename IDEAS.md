...
- 2026-09-26: The fit test (and QA/preflight) passed five chapter openers whose last lines run into the bottom margin and over the folio (padel book p014, p033, p050, p064, p071); check that body text stays above the bottom margin, not only that it is present
- 2026-09-26: After approved pages are revised and re-approved, `status` still reports the interior "ready" and `next` skips re-assembly, although output/interior.pdf still holds the old pages (padel book p014, p033, p050, p064, p071); a revision should mark the assembled interior and preflight stale
